class Game_Status():
    ...
    def get_castling_moves(self, moves):
        ...
        # print('get_casling_moves: ', self.castle_possible_wK, self.castle_possible_wQ, self.castle_possible_white)
    ...
    def remove_invalid_moves(self, moves):
        ...
        for i in range(len(moves) - 1, -1, -1):
            move = moves[i]
            # まず自分を動かしてみる。
            # print('remove_invalid_moves ',move[0], move[1] )
    ...
    def confirm_check(self):
        ...
        # print('confirm_check ', king_location)
    ...
    def confirm_draw(self):
        ...
        for row in range(len(self.board)):
            for col in range(len(self.board[row])):
                ...
                # ポーン、クイーン、ルークが黒白関係なくボード上に1つでもあれば、ドローではない。
                if square[1] == 'P' or square[1] == 'Q' or square[1] == 'R':
                    return False
                else:
                    if square == 'wB':
                        count_wB += 1
                        # 同色のビショップが2枚、or 同色のビショップ1枚＋ナイト1枚あれば、ドローではない。
    ...
    def confirm_draw_threefold(self):

        self.threefold = []
        for row in range(len(self.board)):
            for col in range(len(self.board[row])):
                self.threefold.append(self.board[row][col])
        self.threefoldLog.append(self.threefold)
        # print(self.threefoldLog)

        for item in self.threefoldLog:
            if self.threefoldLog.count(item) > 2:
                self.drawthreefold = True

    def confirm_draw_50rule(self):
        pass

    '''
    2022.05.19
    ２つの「引き分け」を追加する。
    （1）千日手：同じ局面が3回あらわれた場合。連続3回ではない。
        手が進むたびに（make_move）、64マスを配列にセットする。空欄はブランク。
        その配列を、Log配列にセットする。
        Log配列の中に、同じ配列（要素）が3つ存在したらエラーとする。
        本当は、コマがとられる＆ポーンが動く、が発生したら再現性はないので削除してもいいけども、
        Z buttonがあるので、削除は不可
        
    （2）５０手：コマがとられない（白黒50）＆ポーンも動かない
        make_moveで、piece_captured == '--' and piece_moved[1] !='P' -> draw_50_count += 1
        if draw_50_count == 100:
            draw_50_flag = True
        undo_moveで、piece_captured == '--' and piece_moved[1] !='P'　→ draw_50_count -= 1
        __init__で、draw_50_flag = False
    '''
        ...
        for row_num in range(row-1, -1, -1):
            if col + (row - row_num) < 8:
                ...
                if self.board[end_row][end_col] == '--':
                    move = (str(row) + str(col), str(end_row) + str(end_col))
                    moves.append(move)
                # 同じ色のピースがあった場合　→　そこを含まない、かつ、それ以上先には進めないので breakする
                elif self.board[end_row][end_col][0] == self.board[row][col][0]:
                    break
                # 違う色のピースがあった場合　→　そこを含む（取る）、かつ、そこで終わるのでbreakする
                else:
                    move = (str(row) + str(col), str(end_row) + str(end_col))
                    moves.append(move)
                    break
    ...
    def get_Rook_moves(self, row, col, moves):
        # 白:アップ, 黒:ダウン
        for row_num in range(row-1, -1, -1): # row-1からゼロまで、１減らしていく
            end_row = row_num
            end_col = col
            # 空欄だった場合　→　そこを含む
            if self.board[end_row][end_col] == '--':
